			...
			<tbody>
				@foreach($vendors as $key => $value)
					<tr>
						...
						<td>{{ $value->status }}</td>
						<td>{{ $value->vendor_type_id }}</td>
						
						<td>
							<!-- delete the vendor -->
							{{ Form::open(
								array(
									'url' => 'maintenance/maintain_vendors/vendor_accounts/' . $value->log,
									'class' => 'pull-right'
								)
							)}}
							...
							{{ Form::close() }}
							<!-- show the vendor -->
							<a href="{{ URL::to('maintenance/maintain_vendors/vendor_accounts/' . $value->log) }}" class="btn btn-success">Show this account</a>
							<a href="{{ URL::to('maintenance/maintain_vendors/vendor_accounts/' . $value->log) . '/edit' }}" class="btn btn-info">Edit this account</a>
